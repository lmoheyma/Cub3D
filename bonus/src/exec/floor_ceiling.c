...
/*   Updated: 2024/02/19 10:22:19 by aleite-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_buffer(t_cub3d *cub, int y)
{
	...
	while (x < WIDTH)
	{
		...
		cub->f_c->t_x = (int)(SQUARE * (cub->f_c->floor_x
					- cub->f_c->cell_x)) & (SQUARE - 1);
		cub->f_c->t_y = (int)(SQUARE * (cub->f_c->floor_y
					- cub->f_c->cell_y)) & (SQUARE - 1);
		...
		color = cub->param->textures[14][SQUARE * cub->f_c->t_y
			+ cub->f_c->t_x];
		color = (color >> 1) & 8355711;
...
void	floor_ceiling(t_cub3d *cub)
{
	...
	while (y < HEIGHT)
	{
		...
		cub->f_c->floor_step_x = cub->f_c->row_distance * (cub->f_c->raydir_x1
				- cub->f_c->raydir_x0) / WIDTH;
		cub->f_c->floor_step_y = cub->f_c->row_distance * (cub->f_c->raydir_y1
				- cub->f_c->raydir_y0) / WIDTH;
		cub->f_c->floor_x = cub->player->p_x + cub->f_c->row_distance
			* cub->f_c->raydir_x0;
		cub->f_c->floor_y = cub->player->p_y + cub->f_c->row_distance
			* cub->f_c->raydir_y0;
		fill_buffer(cub, y);
		y++;
	}
